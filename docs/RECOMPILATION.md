...
Run `mk7-run` with the same external CIA and ctrtool. The runner verifies and extracts locally, initializes the CTR memory map, executes the generated entry closure, then keeps a resizable SDL3/Vulkan diagnostic window alive. The current 5% closure registers 2,032 functions, compiles 287,232 mapped bytes, and reaches the real startup `SVC 0x21` at guest PC `0x00101564`. The diagnostic top and bottom screens visualize runtime and input state but do not yet contain game graphics.
