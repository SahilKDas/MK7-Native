...
bus_write_u32(0x200,0x3a767273u);bus_write_u8(0x204,0);g_cpu.R[1]=0x200;runtime_swi(0x2d);assert(g_cpu.R[0]==0);const auto srv_handle=g_cpu.R[1];
 runtime_coproc_write(15,0,13,0,3,0x300);bus_write_u32(0x380,0x00010002u);g_cpu.R[0]=srv_handle;runtime_swi(0x32);assert(g_cpu.R[0]==0&&bus_read_u32(0x384)==0);
  bus_write_u32(0x380,0x00020000u);g_cpu.R[0]=srv_handle;runtime_swi(0x32);assert(bus_read_u32(0x384)==0&&bus_read_u32(0x38c)>=0x100u);bus_write_u32(0x380,0x00090040u);g_cpu.R[0]=srv_handle;runtime_swi(0x32);assert(bus_read_u32(0x384)==0);bus_write_u32(0x380,0x000b0000u);g_cpu.R[0]=srv_handle;runtime_swi(0x32);assert(bus_read_u32(0x384)==0&&bus_read_u32(0x388)==0);
 ...
 const auto gsp=get_service("gsp::Gpu");bus_write_u32(0x38c,event);ipc(gsp,0x0013);assert(bus_read_u32(0x384)==0x2a07u&&bus_read_u32(0x390)>=0x100);const auto gsp_shared=bus_read_u32(0x390);g_cpu.R[0]=gsp_shared;g_cpu.R[1]=0x1000;runtime_swi(0x1f);assert(g_cpu.R[0]==0);ipc(gsp,0x0016);assert(bus_read_u32(0x384)==0);bus_write_u32(0x1800,0x100);bus_write_u32(0x1820,1);bus_write_u32(0x1824,0x3000);bus_write_u32(0x1828,8);bus_write_u32(0x3000,1);bus_write_u32(0x3004,0x000f022e);ipc(gsp,0x000c);assert(pica200_snapshot().draw_calls==1&&bus_read_u32(0x1800)==1);
 const auto fs=get_service("fs:USER");ipc(fs,0x0861);assert(bus_read_u32(0x384)==0);bus_write_u32(0x384,7);ipc(fs,0x080c);assert(bus_read_u32(0x384)==0);const auto archive_lo=bus_read_u32(0x388),archive_hi=bus_read_u32(0x38c);
 const char file_name[]="test.bin";for(std::size_t i=0;i<sizeof(file_name);++i)bus_write_u16(0x500+std::uint32_t(i*2),std::uint16_t(file_name[i]));
 bus_write_u32(0x388,archive_lo);bus_write_u32(0x38c,archive_hi);bus_write_u32(0x390,4);bus_write_u32(0x394,sizeof(file_name)*2);bus_write_u32(0x39c,4);bus_write_u32(0x3a0,0);bus_write_u32(0x3a8,0x500);ipc(fs,0x0808);assert(bus_read_u32(0x384)==0);
 bus_write_u32(0x384,0);bus_write_u32(0x388,archive_lo);bus_write_u32(0x38c,archive_hi);bus_write_u32(0x390,4);bus_write_u32(0x394,sizeof(file_name)*2);bus_write_u32(0x398,3);bus_write_u32(0x39c,0);bus_write_u32(0x3a4,0x500);ipc(fs,0x0802);const auto file=bus_read_u32(0x38c);assert(file>=0x100u);
 bus_write_u32(0x510,0x44332211);bus_write_u32(0x384,0);bus_write_u32(0x388,0);bus_write_u32(0x38c,4);bus_write_u32(0x390,1);bus_write_u32(0x398,0x510);ipc(file,0x0803);assert(bus_read_u32(0x384)==0&&bus_read_u32(0x388)==4);
 ipc(file,0x0804);assert(bus_read_u32(0x384)==0&&bus_read_u32(0x388)==4&&bus_read_u32(0x38c)==0);bus_write_u32(0x384,0);bus_write_u32(0x388,0);bus_write_u32(0x38c,4);bus_write_u32(0x394,0x520);ipc(file,0x0802);assert(bus_read_u32(0x384)==0&&bus_read_u32(0x388)==4&&bus_read_u32(0x520)==0x44332211);ipc(file,0x0808);assert(bus_read_u32(0x384)==0);
 bus_write_u32(0x384,archive_lo);bus_write_u32(0x388,archive_hi);ipc(fs,0x080e);assert(bus_read_u32(0x384)==0); const auto apt=get_service("APT:U");ipc(apt,0x0002);assert(bus_read_u32(0x384)==0&&bus_read_u32(0x38c)>=0x100&&bus_read_u32(0x390)>=0x100);ipc(apt,0x0003);assert(bus_read_u32(0x384)==0);ipc(apt,0x0043);assert(bus_read_u32(0x384)==0);ipc(apt,0x0009);assert(bus_read_u32(0x384)==0&&bus_read_u32(0x388)==1);ipc(apt,0x000d);assert(bus_read_u32(0x384)==0&&bus_read_u32(0x388)==0x101u&&bus_read_u32(0x38c)==1u&&bus_read_u32(0x398)==0u);ipc(apt,0x0022);assert(bus_read_u32(0x384)==0);ipc(apt,0x003b);assert(bus_read_u32(0x384)==0);bus_write_u32(0x388,45);ipc(apt,0x004f);ipc(apt,0x0050);assert(bus_read_u32(0x388)==45);ipc(apt,0x0004);assert(bus_read_u32(0x384)==0);
 ...
 bus_write_u32(0x240,0xffff8000u);g_cpu.R[0]=0xffffffffu;g_cpu.R[1]=0x240;g_cpu.R[2]=1;g_cpu.R[3]=0;g_cpu.R[4]=0xffffffffu;runtime_swi(0x25);assert(g_cpu.R[0]==0&&g_cpu.R[1]==0&&ctr_runtime_reschedule_requested());
 g_cpu.R[0]=0xffff8001u;g_cpu.R[1]=0x10001u;runtime_swi(0x3c);assert(g_cpu.R[0]==0&&g_cpu.R[1]==0&&g_cpu.R[2]==0);g_cpu.R[1]=0xffff8000u;runtime_swi(0x0b);assert(g_cpu.R[0]==0&&g_cpu.R[1]==0x30u);g_cpu.R[1]=0xffff8000u;runtime_swi(0x37);assert(g_cpu.R[0]==0&&g_cpu.R[1]==1u);
 g_cpu.R[0]=0xfffff40u;g_cpu.R[1]=0xffff8001u;runtime_swi(0x38);assert(g_cpu.R[0]==0);const auto resource_limit=g_cpu.R[1];bus_write_u32(0x250,1);g_cpu.R[0]=0x260;g_cpu.R[1]=resource_limit;g_cpu.R[2]=0x250;g_cpu.R[3]=1;runtime_swi(0x39);assert(g_cpu.R[0]==0&&bus_read_u32(0x260)==0x04000000u);runtime_swi(0x3a);assert(g_cpu.R[0]==0&&bus_read_u32(0x260)>=0x1000u);
 g_cpu={};g_cpu.cpsr=0x10;g_cpu.R[15]=0x1000;g_cpu.R[13]=0x3000;runtime_coproc_write(15,0,13,0,3,0x200);ctr_runtime_scheduler_initialize();g_cpu.R[1]=0x2001;g_cpu.R[2]=0xabcdef01;g_cpu.R[3]=0x3ff0;runtime_swi(0x8);assert(g_cpu.R[0]==0);assert(ctr_runtime_scheduler_rotate());assert(g_cpu.R[15]==0x2000&&g_cpu.R[0]==0xabcdef01&&g_cpu.R[13]==0x3ff0&&(g_cpu.cpsr&CPSR_T_BIT));runtime_coproc_write(15,0,13,0,3,0x280);runtime_swi(0x9);assert(ctr_runtime_reschedule_requested());assert(ctr_runtime_scheduler_rotate());assert(g_cpu.R[15]==0x1000&&runtime_coproc_read(15,0,13,0,3)==0x200);
 g_cpu.R[1]=0;runtime_swi(0x17);const auto wait_event0=g_cpu.R[1];g_cpu.R[1]=0;runtime_swi(0x17);const auto wait_event1=g_cpu.R[1];g_cpu.R[1]=0x2200;g_cpu.R[2]=0;g_cpu.R[3]=0x3f00;runtime_swi(0x8);bus_write_u32(0x270,wait_event0);bus_write_u32(0x274,wait_event1);g_cpu.R[1]=0x270;g_cpu.R[2]=2;g_cpu.R[3]=0;runtime_swi(0x25);assert(ctr_runtime_scheduler_rotate());g_cpu.R[0]=wait_event1;runtime_swi(0x18);assert(ctr_runtime_scheduler_rotate());assert(g_cpu.R[0]==0&&g_cpu.R[1]==1);
