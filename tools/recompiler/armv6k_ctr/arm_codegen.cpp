...
// CTR combine HAS_DATA (ctr_cycle_combine): true for every op that makes a
// data-bus (non-code) access — selects melonDS's AddCycles_CD/CDI vs C/CI.
bool ctr_has_data(IrOp op) {
    switch (op) {
        case IrOp::LDR:  case IrOp::LDRB: case IrOp::LDRH:
        case IrOp::LDRSB: case IrOp::LDRSH:
...
// CTR AddCycles_CDI (loads/read-modify-write) has one internal cycle and
// slightly different main-RAM overlap from AddCycles_CD (stores).
bool ctr_is_load(IrOp op) {
    switch (op) {
        case IrOp::LDR:  case IrOp::LDRB: case IrOp::LDRH:
        case IrOp::LDRSB: case IrOp::LDRSH:
...
// Static CTR internal-cycle count (numI) for the combine — melonDS
// ARMInterpreter*.cpp: +1 for a register-specified shift, a FLAT 1 (S=0) or
// 3 (S=1) for the classic MUL family (NOT operand-dependent; the CTR-only
// mul_wait_cycles path is untouched), +2 for MCR, +3 for MRC. Everything else
// (including the ARMv5
// signed xy-multiply family,
// which really is single-cycle on CTR) is 0.
uint32_t ctr_internal_cost(const Instr& ins) {
    uint32_t n = 0u;
...
// CTR cycle-combine accumulators (ctr_cycle_combine — see runtime_arm.h).
...
// static internal-cycle count (ctr_internal_cost, below), assigned once
...
// _cyc and are dead code on the CTR path (never referenced there). BL /
...
// second time (see ctr_tick_expr's doc comment below).
...
// (recompiler/armv4t/runtime_arm.h: ctr_code_cycles + CTR_CTR_CODE_K) rather
// than a runtime_code_cycles(pc) call: for a fixed pc and instruction-set
// state the four possible values — one per CTR code-timing class — are
// known here, so the emitted constant carries them and the runtime only
// supplies which class is in force. ctr_code_cycles returns EXACTLY what
// runtime_code_cycles(pc) returns (pinned by
// runner/tests/code_cycles_fold_test.cpp) and falls back to that call for
// the CTR and stale-publication cases. beads-yjp.70 phase 2A.
//
...
std::string code_numc_expr(uint32_t pc, bool) {
    return "ctr_code_cycles(" + fmt_hex32(pc) + ")";
}
...
// Emit the CTR-vs-CTR `runtime_tick` ternary argument. The CTR branch is
// ALWAYS `cyc_var` verbatim — the original flat expression, unchanged. The
// CTR branch calls ctr_cycle_combine with this instruction's OWN numC
...
// ctr_refill_cycles(target) on top of the combine. `code_expr` is normally
// the `_code_<pc>`
...
std::string ctr_tick_expr_ex(const std::string& code_expr,
                              const std::string&,
                              const std::string& data_var,
                              const std::string& int_var, bool has_data,
                              bool,
                              const std::string& refill_target_expr) {
    std::string result = "ctr_cycle_combine(" + code_expr + ", " + data_var +
                         ", " + int_var + ", " + (has_data ? "1u" : "0u") + ")";
    if (!refill_target_expr.empty()) {
        result += " + ctr_refill_cycles(" + refill_target_expr + ")";
    }
    return result;
}
std::string ctr_tick_expr(const Instr& ins, bool has_data,
                           const std::string& refill_target_expr = "") {
    return ctr_tick_expr_ex(code_var_for(ins), cyc_var_for(ins),
...
std::string branch_tick_expr(const std::string& refill_target_expr) {
    return "ctr_refill_cycles(" + refill_target_expr + ")";
}
...
std::string emit_direct_branch(uint32_t target, uint32_t branch_pc,
    ...
    // CTR pipeline-refill term uses the static target literal directly.
    ...
    // five-instruction MRS/MSR `bx lr` leaves that the MPH CTR fight calls
    ...
    // CTR_INLINE_LEAVES=0 — takes the unchanged link call below, which
    ...
    if (leaf) {
        ...
        const std::string transfer = ctx.trace_live_transfers
            ? (std::string("runtime_live_transfer(") + fmt_hex32(branch_pc) +
               ", " + fmt_hex32(target) + ", CTR_LIVE_TRANSFER_BL);")
            : std::string();
        s << indent << "{ static CtrLinkSlot " << slot << " = {0, 0u, "
    ...
    if (is_link) {
        ...
        // below) evaluates ctr_cycle_combine(0, 0, 0, 0) = 0 for CTR too,
    ...
    if (name) {
        ...
        if (!is_link && target == ctx.current_function_addr) {
            ...
            // unregisters. CTR_DIRECT_LINK=0 makes runtime_link_* behave
...
bool emit_data_processing(std::ostringstream& body, const Instr& ins,
    ...
    if (ins.set_flags) {
        if (excpt_return) {
            ...
            const std::string exception_tick =
                "ctr_cycle_combine(" + code_var_for(ins) + ", 0u, " +
                int_var_for(ins) + ", false) + ctr_refill_cycles(" +
                return_target + ")";
    ...
    if (!is_test) {
        if (ins.rd == 15) {
            ...
            // the +2 refill folded into _cyc for CTR) before transferring.
            // CTR: DP-to-PC never interworks (CPSR.T is unchanged), so
            // pc_var is already the correct-mode target for the refill.
            body << indent << "runtime_tick("
                 << ctr_tick_expr(ins, false,
                                   "(" + pc_var + (ins.thumb ? " | 1u)" : ")"))
                 << ");\n";
...
bool emit_branch(std::ostringstream& body, const Instr& ins,
    ...
    switch (ins.op) {
        ...
        case IrOp::BX: {
            ...
            // for the CTR refill's target-numC is set AFTER this tick (by
        ...
        case IrOp::BLX_reg: {
            ...
            // CTR too, not re-derive a fresh nonzero numC — see
            // code_var_for's doc comment.
        ...
        case IrOp::BLX_imm: {
            ...
            // BLX_imm's target is a compile-time constant, so the CTR
        ...
        case IrOp::BL_suffix: {
            ...
            // callee C-returns) does not double-count — for CTR that's just
            // _cyc; for CTR it's _cyc/_code/_data/_int together, so the
            ...
            if (ctx.trace_live_transfers)
                body << indent << "runtime_live_transfer(" << fmt_hex32(ins.pc)
                     << ", " << target_var << ", "
                     << (blx ? "CTR_LIVE_TRANSFER_BLX_REG"
                             : "CTR_LIVE_TRANSFER_BL")
                     << ");\n";
            body << indent
...
bool emit_memory(std::ostringstream& body, const Instr& ins,
    ...
    if (is_load) {
        ...
        if (ins.rd == 15) {
            ...
            // CTR refill's numC(target) sees the correct mode — no
            // staleness caveat here (unlike the BX/BLX family).
            body << indent << "runtime_tick("
                 << ctr_tick_expr(ins, true, val_var)
                 << ");\n";
            body << indent << "if (runtime_unwinding()) return;\n";
            if (ctx.trace_live_transfers)
                body << indent << "runtime_live_transfer(" << fmt_hex32(ins.pc)
                     << ", " << val_var
                     << ", CTR_LIVE_TRANSFER_LDR_PC);\n";
...
bool emit_block_transfer(std::ostringstream& body, const Instr& ins,
    ...
    if (blk.reg_list == 0) {
        ...
        if (blk.load) {
            ...
            body << indent << "runtime_tick("
                 << ctr_tick_expr(ins, true, pcv)
                 << ");\n";
            body << indent << "if (runtime_unwinding()) return;\n";
            if (ctx.trace_live_transfers)
                body << indent << "runtime_live_transfer(" << fmt_hex32(ins.pc)
                     << ", " << pcv
                     << ", CTR_LIVE_TRANSFER_LDM_PC);\n";
...
bool emit_coprocessor(std::ostringstream& body, const Instr& ins,
                      const char* indent) {
    // CTR CP15 (and any future coprocessor) access. The register
    ...
    // runtime drives the real CTR bus configuration from these writes.
...
std::string ArmCodegen::emit_instr(const Instr& ins, const CodegenCtx& ctx,
    ...
    if (cond_never(ins.cond)) {
        ...
        // immediately). CTR combines as class C (numI=0, has_data=0 ->
        // cost=numC) with a literal "1u" CTR fallback — the identical value
        // the old flat `runtime_tick(1u)` used for both CPUs, so CTR is
        // byte-for-byte unchanged.
        s << "    runtime_tick("
          << ctr_tick_expr_ex(code_numc_expr(ins.pc, ins.thumb),
    ...
    // bump is inlined (one increment; CTR_STATIC_CPU folds the index) and the
    ...
    // CTR combine accumulators (ctr_cycle_combine — melonDS AddCycles_C/
    ...
    if (ins.op != IrOp::SWI) {
        ...
        os << indent << int_var << " = " << ctr_internal_cost(ins) << "u;\n";
    }
    ...
    os << "    runtime_tick("
       << ctr_tick_expr(ins, ctr_has_data(ins.op)) << ");\n";
